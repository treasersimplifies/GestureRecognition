...
{					     
  ...
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  GPIO_Init(GPIOC, &GPIO_InitStructure);					    //根据设定参数初始化GPIO 
  GPIO_SetBits(GPIOC,GPIO_Pin_6|GPIO_Pin_7);				  //PC4,PC5 输出高	
}
...
void ZFDC_IIC_Start(void)
{
	ZFDC_SDA_OUT();     //sda线输出
	ZFDC_IIC_SDA=1;	  	  
	ZFDC_IIC_SCL=1;
	ZFDC_IIC_Delay();
 	ZFDC_IIC_SDA=0;//START:when CLK is high,DATA change form high to low 
	ZFDC_IIC_Delay();
	ZFDC_IIC_SCL=0;//钳住I2C总线，准备发送或接收数据 
}
//产生IIC停止信号
...
void ZFDC_IIC_Stop(void)
{
	ZFDC_SDA_OUT();//sda线输出
	ZFDC_IIC_SCL=0;
	ZFDC_IIC_SDA=0;//STOP:when CLK is high DATA change form low to high
 	ZFDC_IIC_Delay();
	ZFDC_IIC_SCL=1; 
	ZFDC_IIC_SDA=1;//发送I2C总线结束信号
	ZFDC_IIC_Delay();							   	
}
//等待应答信号到来
//返回值：1，接收应答失败
...
u8 ZFDC_IIC_Wait_Ack(void)
{
	u8 ucErrTime=0;
	ZFDC_SDA_IN();      //SDA设置为输入  
	ZFDC_IIC_SDA=1;ZFDC_IIC_Delay();	   
	ZFDC_IIC_SCL=1;ZFDC_IIC_Delay();	 
	while(ZFDC_READ_SDA)
	{
		ucErrTime++;
		if(ucErrTime>250)
...
void ZFDC_IIC_Send_Byte(u8 txd)
{                        
  u8 t;   
	ZFDC_SDA_OUT(); 	    
    ZFDC_IIC_SCL=0;//拉低时钟开始数据传输
    for(t=0;t<8;t++)
    {              
        ZFDC_IIC_SDA=(txd&0x80)>>7;
        txd<<=1; 	  
		    ZFDC_IIC_SCL=1;
		    ZFDC_IIC_Delay(); 
		    ZFDC_IIC_SCL=0;	
...
u8 ZFDC_IIC_Read_Byte(unsigned char ack)
{
	unsigned char i,receive=0;
	ZFDC_SDA_IN();//SDA设置为输入
    for(i=0;i<8;i++ )
	{
		...
		ZFDC_IIC_SCL=1;
        receive<<=1;
        if(ZFDC_READ_SDA)receive++;   
		ZFDC_IIC_Delay(); 
    }				 
    if (!ack)
        ZFDC_IIC_NAck();//发送nACK
    else
        ZFDC_IIC_Ack(); //发送ACK   
    return receive;
}
...
{ 
    ZFDC_IIC_Start(); 
	ZFDC_IIC_Send_Byte((FDC2214_ADDR<<1)|0);//发送器件地址+写命令	
	if(ZFDC_IIC_Wait_Ack())	//等待应答
	{
		ZFDC_IIC_Stop();		 
		return 1;		
	}
    ZFDC_IIC_Send_Byte(reg);//写寄存器地址
    ZFDC_IIC_Wait_Ack();		 //等待应答 
	ZFDC_IIC_Send_Byte(MSB);  //发送数据1
	if(ZFDC_IIC_Wait_Ack())	 //等待ACK
	{
		ZFDC_IIC_Stop();	 
		return 1;		 
	}		 
	ZFDC_IIC_Send_Byte(LSB);  //发送数据2
	if(ZFDC_IIC_Wait_Ack())	 //等待ACK
	{
		ZFDC_IIC_Stop();	 
		return 1;		 
...
u16 ZFDC_Read(u8 reg)
{
    ...
    ZFDC_IIC_Wait_Ack();		//等待应答
    ZFDC_IIC_Start();
	ZFDC_IIC_Send_Byte((FDC2214_ADDR<<1)|1);//发送器件地址+读命令	
    ZFDC_IIC_Wait_Ack();		//等待应答 
	res=ZFDC_IIC_Read_Byte(1)<<8;//读取数据,发送ACK
	  
	res|=ZFDC_IIC_Read_Byte(0);//读取数据,发送nACK
    ZFDC_IIC_Stop();			//产生一个停止条件
	return res;		
}
/*得到通道x转换后的数据
 *index:0 -> CH0
...
u32 FCD2214_ReadCH(u8 index) 
{
	...
	switch(index)
	{
		...
		case 1:
		  ...
		  result = (result<<16)|(FDC_Read(DATA_LSB_CH1));//0
			break;
		case 2:
...
u32 ZFCD2214_ReadCH(u8 index) 
{
	...
	{
		case 0:
		  result = ZFDC_Read(DATA_CH0)&0x0FFF;
		  result = (result<<16)|(ZFDC_Read(DATA_LSB_CH0));
			break;
		case 1:
		  result = ZFDC_Read(DATA_CH1)&0x0FFF;
		  result = (result<<16)|(ZFDC_Read(DATA_LSB_CH1));//0
			break;
		case 2:
		  result = ZFDC_Read(DATA_CH2)&0x0FFF;
		  result = (result<<16)|(ZFDC_Read(DATA_LSB_CH2));
			break;
		case 3:
		  result = ZFDC_Read(DATA_CH3)&0x0FFF;
		  result = (result<<16)|(ZFDC_Read(DATA_LSB_CH3));
			break;
		default:break;
	}
	result =result&0x0FFFFFFF;
...
u8 FDC2214_Init(void)
{
	...
	if(res==0x5449)
	{
		...
		Set_FDC2214(MUX_CONFIG,0xC2,0x0D);//yes,通道0，1，2 ，3；选择10Mhz为超过振荡槽振荡频率的最低设置，多通道，四通道
...
u8 ZFDC2214_Init(void)
{
	...
	//检测ID是否正确，再初始化寄存器
	res=ZFDC_Read(MANUFACTURER_ID);
	if(res==0x5449)
	{
		//设置Set_FDC2214寄存器
		ZSet_FDC2214(RCOUNT_CH0,0x34,0xFB);//参考计数转换间隔时间（T=(RCOUNT_CH0*16)/Frefx）
		ZSet_FDC2214(RCOUNT_CH1,0x34,0xFB);
		ZSet_FDC2214(RCOUNT_CH2,0x34,0xFB);
		//
		ZSet_FDC2214(RCOUNT_CH3,0x34,0xFB);
		
		ZSet_FDC2214(SETTLECOUNT_CH0,0x00,0x1B);//转换之前的稳定时间（T=(SETTLECOUNT_CHx*16)/Frefx）
		ZSet_FDC2214(SETTLECOUNT_CH1,0x00,0x1B);
		ZSet_FDC2214(SETTLECOUNT_CH2,0x00,0x1B);
		//
		ZSet_FDC2214(SETTLECOUNT_CH3,0x00,0x1B);
		
		ZSet_FDC2214(CLOCK_DIVIDERS_C_CH0,0x20,0x02);//选择在0.01MHz ~ 10MHz的传感器频率
		ZSet_FDC2214(CLOCK_DIVIDERS_C_CH1,0x20,0x02);//Frefx = Fclk = 43.4MHz/2(2分频)
		ZSet_FDC2214(CLOCK_DIVIDERS_C_CH2,0x20,0x02);//CHx_REF_DIVIDER=2;CHx_FIN_SEL=2
		//
		ZSet_FDC2214(CLOCK_DIVIDERS_C_CH3,0x20,0x02);
		
		ZSet_FDC2214(DRIVE_CURRENT_CH0,0x78,0x00);//0.146ma（传感器时钟建立+转换时间的驱动电流）
		ZSet_FDC2214(DRIVE_CURRENT_CH1,0x78,0x00);
		ZSet_FDC2214(DRIVE_CURRENT_CH2,0x78,0x00);
		ZSet_FDC2214(DRIVE_CURRENT_CH3,0x78,0x00);
		
		ZSet_FDC2214(ERROR_CONFIG,0x00,0x00);//全部禁止错误汇报
		
		ZSet_FDC2214(MUX_CONFIG,0xC2,0x0D);//yes,通道0，1，2 ，3；选择10Mhz为超过振荡槽振荡频率的最低设置，多通道，四通道
		
		ZSet_FDC2214(CONFIG,0x14,0x01);//激活模式，使用内部振荡器做参考频率，INTB引脚会随状态寄存器更新被置位
	}
	else return 1;
...
{
	float Cap;
	Data_FDC = ZFCD2214_ReadCH(index);
//	Cap = 56645.763f/((float)Data_FDC);
//	return ((Cap*Cap)-33);
	Cap = 232021045.248/(Data_FDC);
	return (Cap*Cap);
